...
from mustard.prepare_mustard_error_analysis import get_testdataset_segments_labels
        ...
        while True:
            ...
            for batch_data in dataloader['train']:
                ...
                if self.args.speaker :
                    speaker = batch_data['speaker'].to(self.args.device)

                labels = batch_data['labels'][self.args.tasks].to(self.args.device).view(-1, 1)
                optimizer.zero_grad()
                if self.args.speaker :
                    outputs = model(text, audio, vision, speaker)
                else:
