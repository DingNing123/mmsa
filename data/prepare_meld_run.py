...
def run_normal(seeds):
    ...
    save_path = os.path.join(args.res_save_path,
                             args.datasetName + '-' + args.modelName + '-' + args.tasks
                             + '.csv') # 需要修改load_data.py 中的index = 5
    if not os.path.exists(args.res_save_path):
        os.makedirs(args.res_save_path)
...
def parse_args():
    ...
    parser.add_argument('--datasetName', type=str, default='meld',
                        help='support mosi/sims/mustard/meld')
    ...
    parser.add_argument('--data_dir', type=str, default='/media/dn/85E803050C839C68/m_fusion_data/bert_meld/',
