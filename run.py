...
def run_normal(seeds):
    ...
    for i, seed in enumerate(seeds):
        ...
        args['seed'] = seed
        print()
        print('Start running %s. times : %d ' % (args.modelName,args.cur_time))
        test_results = run(args)
        model_results.append(test_results)
    # save results
    #criterions = list(model_results[0][args.tasks[0]].keys())
    criterions = ['precision','recall','f1_score']
    print('create pd.DataFrame a csv to save results: ')

    df = pd.DataFrame(columns=["Model"] + criterions)

    print()
    print(f'{args.modelName} {args.variant} - {args.modality} + context: {str(args.context)} + speaker: {str(args.speaker)}')
    for m in args.tasks:
    ...
    save_path = os.path.join(args.res_save_path,
                             args.datasetName + '-' + args.modelName + args.variant  + '-' 
                             + args.modality+ '-' + args.tasks + '-' + args.split + 
    ...
    print('normal results are saved to %s ...' % (save_path))
...
def parse_args():
    ...
    parser.add_argument('--context', type=bool, default=False,
                        help=' + context')
    parser.add_argument('--speaker', type=bool, default=False,
                        help=' + speaker')
    parser.add_argument('--modelName', type=str, default='lf_dnn1',
                        help='support mult/tfn/lmf/mfn/ef_lstm/lf_dnn/lf_dnn1/mtfn/mlmf/mlf_dnn')
    # use mustard with context and speaker mustard_33_context.pkl
    ...
    parser.add_argument('--tasks', type=str, default='M',
                        help='M/T/A/V/MTAV/...')
    parser.add_argument('--split', type=str, default='dep',
                        help='our_i/src_i/dep')
    # dep : dependent,src_i: independent,our_i:oursplit independent
    parser.add_argument('--modality', type=str, default='tav',
                        help='t/a/v/tav/ta/tv/av')
    # variant : the paper v0, v1, v2
    parser.add_argument('--variant', type=str, default='v2',
                        help='v0/v1/v2')
    parser.add_argument('--num_workers', type=int, default=0,
                        help='num workers of loading data')
    parser.add_argument('--model_save_path', type=str, default='../mmsa_results/model_saves',
                        help='path to save model.')
    parser.add_argument('--res_save_path', type=str, default='../mmsa_results/result_saves',
                        help='path to save results.')
    parser.add_argument('--data_dir', type=str, default=M_FUSION_DATA_PATH + '/bert_mustard/',
