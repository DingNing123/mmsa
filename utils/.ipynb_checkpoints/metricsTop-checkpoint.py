...
class MetricsTop():
    def __init__(self):
        self.metrics_dict = {
            ...
            'MELD': self.__eval_meld_regression,
            'MUSTARD': self.__eval_sims_regression,
            'MUSTARD_OURSPLIT': self.__eval_sims_regression,
    ...
    def __eval_meld_regression(self, y_pred, y_true):
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()
    ...
    def __eval_mosi_regression(self, y_pred, y_true):
        return self.my_eval_mosei_regression(y_pred, y_true)

    def my_eval_mosei_regression(self,y_pred, y_true):
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()
        ...
        # five classes{[-1.0, -0.7], (-0.7, -0.1], (-0.1, 0.1], (0.1, 0.7], (0.7, 1.0]}
        ms_5 = [-3.01, -1.6, -0.1, 0.1, 1.6, 3.01]
        test_preds_a5 = test_preds.copy()
        test_truth_a5 = test_truth.copy()
        for i in range(5):
            test_preds_a5[np.logical_and(test_preds > ms_5[i], test_preds <= ms_5[i + 1])] = i
