...
class Storage(dict):
    """
    A Storage object is like a dictionary except `obj.foo` can be used in adition to `obj['foo']`
...
class Config():
    def __init__(self, args):
        ...
        try:
            self.global_running = vars(args) # convert to dict
        except TypeError:
    ...
    def __datasetCommonParams(self):
        tmp = {
            'mosi':{
                'datapath': os.path.join(self.data_dir, 'mosi.pkl'),
                'input_lens': (21, 20, 20),
                'feature_dims': (768, 74, 47), # (text, audio, video)
            },
            'meld': {
                'datapath': os.path.join(self.data_dir, 'meld.pkl'),
                'input_lens': (18, 1, 1),
                'feature_dims': (768, 600, 600),  # (text, audio, video)
            },
            'mustard': {
                'datapath': os.path.join(self.data_dir, 'mustard_33_context.pkl'),
                'label_dir': os.path.join(self.data_dir, 'csv'),
                'input_lens': (29, 36, 21),
                'feature_dims': (768, 33, 2048),  # (text, audio, video)
            },
            'sims':{
    ...
    def __LF_DNN1(self):
        tmp = {
            ...
            'datasetParas':{
                ...
                'mustard': {
                    'hidden_dims': (128, 16, 128),
                    'speakers': 4,# train speaker num will update
                    'csindex': 4,# text context split index
                    'text_out': 32,
                    'post_fusion_dim1': 256,
                    'post_fusion_dim2': 32,
                    'dropouts': (0.2, 0.2, 0.2, 0.2),
