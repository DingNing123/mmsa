...
class MFN(nn.Module):
    ...
    def get_insize(self):
        self.dh_l,self.dh_a,self.dh_v = self.args.hidden_dims
        ...
        if self.args.modality == 'tv':
            total_h_dim = self.dh_l + self.dh_v
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        for i in range(t):
            ...
            if self.args.modality == 'tv':
                prev_cs = torch.cat([prev_c_l,prev_c_v], dim=1)
                new_cs = torch.cat([new_c_l,new_c_v], dim=1)
            if self.args.modality == 'av':
        ...
        if self.args.modality == 'tv':
            last_hs = torch.cat([last_h_l,last_h_v,last_mem], dim=1)
        if self.args.modality == 'av':
