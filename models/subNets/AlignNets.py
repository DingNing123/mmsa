...
class AlignSubNet(nn.Module):
    def __init__(self, args, mode):
        ...
        # print('Create Class AlignSubNet  construct function')
        # print()
        # print('mode must be one of three avg_pool,ctc conv1d')
        # print('mode:',mode)
        # print()
    ...
    def __avg_pool(self, text_x, audio_x, video_x):
        def align(x):
            ...
            if raw_seq_len // self.dst_len == raw_seq_len / self.dst_len:
                # print('')
                # print('if can divided by dst_len just ')
                pad_len = 0
                ...
                # print('if cannot divided just the pool_size is after add 1 : ')
                # print(pool_size)
                # print('pad_len is -----------')
                # print(pad_len)

            pad_x = x[:, -1, :].unsqueeze(1).expand([x.size(0), pad_len, x.size(-1)])
            # print('select the end frame or word to expand to the pad_len time'
            #       'axis ')
            # print('then we will concat  at dim : 1 ')
            x = torch.cat([x, pad_x], dim=1).view(x.size(0), pool_size, self.dst_len, -1)
            # print('after view to 4 dim then in dim 1 to mean: ----  clever ---------- ')
            x = x.mean(dim=1)
            return x

        # print()
        # print('---------start call __avg_pool in AlignNets.py--------------')
        # print()
        text_x = align(text_x)
        audio_x = align(audio_x)
        video_x = align(video_x)

        # print('after algin the shape will changed to .... I can guess haha. ')
        return text_x, audio_x, video_x
